...
> **Research Completed**: Anthropic Engineering Blog (3 articles), Linear Integration, MCP vs Code Execution
...
- MCP: NOT installed → needs `claude mcp add linear`
- Skill: Available at `/home/director/.agents/skills/linear/`
...
- [ ] `claude mcp login linear` (OAuth authentication)
- [ ] Create "Villa Thaifa Platform" product area
- [ ] Create teams: Engineering, Product, Operations
- [ ] Test `/linear` skill works

**0.2 Knowledge Persistence for Future Instances**
- [ ] Create `memory/knowledge/integrations/linear-guide.md`
- [ ] Create `memory/knowledge/patterns/long-running-agents.md`
- [ ] Create `memory/knowledge/patterns/mcp-code-execution.md`
- [ ] Update CLAUDE.md to reference these at session start

**0.3 Session Startup Protocol**
Add to CLAUDE.md rule:
```markdown
## Session Startup Protocol
1. Read `workstream/active/` for in-progress work
2. Read `memory/knowledge/integrations/` for tool status
3. Check Linear for assigned issues (if configured)
4. Run health check: `npm test && npm run build`
5. Select single feature/task
6. Create branch: `agent/claude/YYYY-MM-DD-task`
```

### Phase 1: Agent & Rules Consolidation (Day 2-3)

**1.1 Centralize Rules (`.agents/rules/`)**
- [ ] `.agents/rules/workspace.md` (Artifacts, Paths)
- [ ] `.agents/rules/git.md` (Branching, Commits)
- [ ] `.agents/rules/verification.md` (Testing, Evidence)
- [ ] `.agents/rules/linear-workflow.md` (Issue lifecycle)

**1.2 Refactor AGENTS.md as Index**
- Keep only: Manifest, References, Active Context
- Move rules to `.agents/rules/`
- Index agents: Gemini, Claude (with links)

**1.3 Unify Agent Configs**
- Merge shared rules from CLAUDE.md/GEMINI.md into AGENTS.md
- Keep only agent-specific personas in individual files
...
**P0 - KNOWLEDGE PERSISTENCE**

Create files for future Claude instances:

| File | Purpose |
|------|---------|
| `memory/knowledge/integrations/linear-guide.md` | How to use Linear with Claude |
| `memory/knowledge/patterns/long-running-agents.md` | Session protocol from Anthropic |
| `memory/knowledge/patterns/mcp-code-execution.md` | Hybrid approach patterns |

**P0 - UPDATE CLAUDE.MD**

Add Session Startup Protocol rule to `~/grid/CLAUDE.md`:
```markdown
## Session Startup Protocol
1. Read `workstream/active/` for in-progress
2. Read `memory/knowledge/integrations/` for tools
3. Check Linear for assigned issues
4. Health check: `npm test && npm run build`
5. Select single feature/task
6. Create branch: `agent/claude/YYYY-MM-DD-task`
```
